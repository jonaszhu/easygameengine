...
#define IDD_MESSAGE_GAME_CENTER_LOGIN_FAIL   505         // 中心服务器登录失败
...
#define IDD_MESSAGE_GET_ONLINE_PLAYERS  708         // 得到在线玩家列表
...
{
	GameServerInfo()
		: GameId(0),ServerPort(0),OnlinePlayerCount(0),lastMoney(0)
	{}
	GameServerInfo(int gi,std::string sn,std::string si,int sp,int opc,int money)
		: GameId(gi),ServerName(sn),ServerIP(si),ServerPort(sp),OnlinePlayerCount(opc),lastMoney(money)
	{}

	int GameId;                   // 游戏ID
	std::string ServerName;       // 游戏服务器名称
	std::string ServerIP;         // 游戏服务器IP
	int ServerPort;               // 游戏服务器端口
