...
public:
	...
	*/
	...
	/// 从指定内存中读取动画文件
	bool OpenAnimatorFromMemory(void *data,DWORD length);
...
private:
	...
	/// 用于解压内存中的所有文件
	int infm(MBytef *source,unsigned char *dest,unsigned long startpos,unsigned long length,unsigned long totalSize,unsigned long *clength);
	/// 用于解压压缩包中的制定文件到内存中
	...
	/// 解密数据
	void Decrypto(unsigned char *data,unsigned long length,unsigned long pos=0);
...
private:
	...
	bool                                    m_isMemory;     // 是否从缓冲区读取

	void*                                   m_Buffer;       // 文件缓冲区
	unsigned long                           m_BufferSize;   // 缓冲区大小
	FILE*                                   m_File;         // 文件读写
};
