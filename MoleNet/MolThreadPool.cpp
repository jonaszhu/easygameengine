...
bool CThreadPool::ThreadExit(Thread * t)
{
	...
	printf("Thread %u entered the free pool.", t->ControlInterface.GetId());
	_mutex.Release();
	return true;
}
...
void CThreadPool::ExecuteTask(ThreadBase * ExecutionTarget)
{
	...
	if(m_freeThreads.size())
	{
		...
		printf("Thread %u left the thread pool.", t->ControlInterface.GetId());
	}
	else
	{
	...
	printf("Thread %u is now executing task at 0x%p.", t->ControlInterface.GetId(), ExecutionTarget);
...
void CThreadPool::Startup()
{
	...
	printf("Startup, launched %u threads.", tcount);
}
...
void CThreadPool::ShutStats()
{
	...
	printf("Active Threads: %u", m_activeThreads.size());
	printf("Suspended Threads: %u", m_freeThreads.size());
	printf("Requested-To-Freed Ratio: %.3f%% (%u/%u)", float( float(_threadsRequestedSinceLastCheck+1) / float(_threadsExitedSinceLastCheck+1) * 100.0f ), _threadsRequestedSinceLastCheck, _threadsExitedSinceLastCheck);
	printf("Eaten Count: %d (negative is bad!)", _threadsEaten);
	printf("============================================");
	_mutex.Release();
}
...
void CThreadPool::IntegrityCheck()
{
	...
	if(gobbled < 0)
	{
		...
		printf("IntegrityCheck: (gobbled < 0) Spawning %u threads.", new_threads);
	}
	else if(gobbled < THREAD_RESERVE)
	{
		...
		printf("IntegrityCheck: (gobbled <= 5) Spawning %u threads.", new_threads);
	}
	else if(gobbled > THREAD_RESERVE)
	{
		...
		printf("IntegrityCheck: (gobbled > 5) Killing %u threads.", kill_count);
	}
	else
	{
		// perfect! we have the ideal number of free threads.
		printf("IntegrityCheck: Perfect!");
	}
...
{
	printf("Killing %u excess threads.", count);
	_mutex.Acquire();
	Thread * t;
...
static unsigned long WINAPI thread_proc(void* param)
{
	...
	for(;;)
	{
		...
		{
			printf("Thread %u exiting.", tid);
			break;
		}
		else
		{
			if(ht)
				printf("Thread %u waiting for a new task.", tid);
