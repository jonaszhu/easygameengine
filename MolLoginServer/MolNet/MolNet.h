 ...
 *
 */
void MOLNETEXPORT InitMolNet(uint32 MaxClients=0,uint32 TimeOut=60,uint32 bufferSize=0);

/** 
 * 卸载网络
 */
void MOLNETEXPORT CleanMolNet(void);
 ...
 */
bool MOLNETEXPORT StartMolNet(const char * ListenAddress, uint32 Port);
 ...
 */
bool MOLNETEXPORT IsConnected(void);
 ...
 */
bool MOLNETEXPORT IsConnected(uint32 index);
 ...
 */
void MOLNETEXPORT Disconnect(uint32 index);
 ...
 */
bool MOLNETEXPORT Send(uint32 index,CMolMessageOut &out);
 ...
 */
std::string MOLNETEXPORT GetIpAddress(uint32 index);
 ...
 * @return 返回得到的消息的个数
 */
int MOLNETEXPORT GetNetMessage(NetMessage & mes);
 ...
 * @param task 我们要执行的任务
 */
void MOLNETEXPORT ExecuteTask(ThreadBase * ExecutionTarget);
